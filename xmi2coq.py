...
for item in items:
    ...
    if item.hasAttribute('association'):
        ...
        for e in elist:
            if e.nodeName == 'lowerValue' or e.nodeName == 'upperValue':
                ...
                if e.hasAttribute('value'):
                    element[key] = 'Star' if e.getAttribute('value')=='*' else '(Nat %s)'%e.getAttribute('value')
                else:
                    element[key] = '(Nat 0)'
        ...
        element['attrtype'] = 'c%s'%idMap[item.getAttribute('type')]
...
for Class in Class_list:
    isAbstract = 'false' if Class['xmi:type'] == 'uml:Class' else 'true'
    fp.write('Definition %s := \n' % (Class["id"]))
    fp.write('  BClass %d "%s" %s. \n' % (idx, Class['name'], isAbstract))
    idx = idx + 1
fp.write('\n')
...
for Attribute in Attribute_list:
    fp.write('Definition %s := \n' % (Attribute["id"]))
    fp.write('  BAttribute %d "%s" %s %s.\n' % (idx, Attribute['name'], Attribute['attrtype'], Attribute['classid']))
    idx = idx + 1
fp.write('\n')
...
for Parameter in Parameter_list:
    fp.write('Definition %s := \n' % (Parameter["id"]))
    fp.write('  BParameter %d "%s" %s.\n' % (idx, Parameter['name'], Parameter['classid']))
    idx = idx + 1
fp.write('\n')
...
for Operation in Operation_list:
    fp.write('Definition %s := \n' % (Operation["id"]))
    fp.write('  BOperation %d \"%s\" [%s] %s.\n' % (idx, Operation['name'], ','.join(Operation['parametersid']), Operation['classid']))
    idx = idx + 1
fp.write('\n')
...
for Association in Association_list:
    fp.write('Definition %s := \n' % (Association["id"]))
    fp.write('  BAssoc %d %s %s none.\n' % (idx, idMap[Association['memberEnd'][1]]['classid'], idMap[Association['memberEnd'][0]]['classid']))
    idx = idx + 1
fp.write('\n')
...
for Rolename in Rolename_list:
    fp.write('Definition %s := \n' % (Rolename["Roleid"]))
    fp.write('  BRole %d "%s" "%s" %s.\n' % (idx, Rolename['nameA'], Rolename['nameB'], Rolename['Associd'] ))
    idx = idx + 1
fp.write('\n')
...
for Multiplicity in Multiplicity_list:
    fp.write('Definition %s := \n' % (Multiplicity["Endid"]))
    fp.write('  BMulti %d %s %s %s %s.\n'%(idx, Multiplicity['Classid'], Multiplicity['Lowval'], Multiplicity['Upval'], Multiplicity['Associd'] ))
    idx = idx + 1
fp.write('\n')

# generalization 部分
for Generalization in Generalization_list:
    fp.write('Definition %s := \n' % (Generalization["Genid"]))
    fp.write('  BGen %d %s %s.\n'%(idx, Generalization['Superid'], Generalization['Subid'] ))
    idx = idx + 1
fp.write('\n')
